// pages/awardCheck/awardCheck.js
const { awardCheck, getAwardDetails } = require("../../../api/api");
Page({
  /**
   * 页面的初始数据
   */
  ...
  async onLoad(options) {
    ...
    if (!content || !score) {
      let taskInfo = await this.getAwardDetails(options._id);
   ...
   * 获取待审核的奖励详情
   */
  getAwardDetails: async function (_id) {
    wx.showLoading({
      title: "加载中",
      mask: true,
    });
    try {
      let res = await getAwardDetails(_id);
  ...
  checkTask: async function (event) {
    ...
    try {
      ...
      let res = await awardCheck({ _id, agree, content, score });
